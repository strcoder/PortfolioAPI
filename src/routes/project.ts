import express from 'express';
import passport from 'passport';
import ProjectServices from '../services/project';
import cacheResponse from '../utils/cacheResponse';
import validationHandler from '../utils/middleware/validationHandler';
import scopesValidationHandler from '../utils/middleware/scopesValidationHandler';
import { FIVE_MINUTES_IN_SECONDS, SIXTY_MINUTES_IN_SECONDS } from '../utils/time';
import { projectIdScehma, createProjectSchema, updateProjectSchema } from '../utils/schemas/project';
import'../utils/auth/strategies/jwt';

const ProjectApi = (app: express.Application) => {
  // const { idSchema, createSchema, updateSchema } = schema;
  // const { getScope, postScope, putScope, deleteScope } = scopes;
  const collection = 'project';
  const router = express.Router();
  const services = new ProjectServices(collection);

  app.use('/api/project', router);

  router.get('/',
    // passport.authenticate('jwt', { session: false }),
    // scopesValidationHandler(['get:project']),

    async (req, res, next) => {
      cacheResponse(res, FIVE_MINUTES_IN_SECONDS);
      try {
        const [objects, size] = await services.getProjects();
        res.status(200).json({
          data: objects,
          size,
          message: `${collection} listed`
        });
      } catch (error) {
        next(error);
      }
    }
  );

  router.get('/:objectId',
    // passport.authenticate('jwt', { session: false }),
    // scopesValidationHandler(getScope),
    validationHandler({ objectId: projectIdScehma }, 'params'),

    async (req, res, next) => {
      cacheResponse(res, SIXTY_MINUTES_IN_SECONDS);
      const { objectId } = req.params;
      try {
        const { project } = await services.getProject({ objectId });
        if (!project || Object.entries(project).length === 0) {
          res.status(404).json({
            data: {},
            message: `${collection} can't recived`,
          });
          return;
        }
        res.status(200).json({
          data: project,
          message: `${collection} recived`,
        });
      } catch (error) {
        next(error);
      }
    }
  );

  router.post('/',
    passport.authenticate('jwt', { session: false }),
    scopesValidationHandler(['create:project']),
    validationHandler(createProjectSchema) ,

    async (req, res, next) => {
      const { body: object } = req;
      try {
        const createdObjectId = await services.createProject({ object });
        res.status(201).json({
          data: createdObjectId,
          message: `${collection} created`
        });
      } catch (error) {
        next(error);
      }
    }
  );

  router.put('/:objectId',
    passport.authenticate('jwt', { session: false }),
    scopesValidationHandler(['update:project']),
    validationHandler({ objectId: projectIdScehma }, 'params'),
    validationHandler(updateProjectSchema),

    async (req, res, next) => {
      const { objectId } = req.params;
      const { body: object } = req;
      try {
        const updateId = await services.updateProject({ objectId: objectId, object });
        res.status(200).json({
          data: updateId,
          message: `${collection} updated`,
        });
      } catch (error) {
        next(error);
      }
    }
  );

  router.delete('/:objectId',
    passport.authenticate('jwt', { session: false }),
    scopesValidationHandler(['delete:project']),
    validationHandler({ objectId: projectIdScehma }, 'params'),

    async (req, res, next) => {
      const { objectId } = req.params;

      try {
        const [deletedId, count] = await services.deleteProject({ objectId });

        if (count === 0) {
          res.status(406).json({
            count,
            data: deletedId,
            message: `${collection} can't deleted`
          });
          return;
        }
        res.status(200).json({
          count,
          data: deletedId,
          message: `${collection} deleted`
        });
      } catch (error) {
        next(error);
      }
    }
  );
}

export default ProjectApi;
